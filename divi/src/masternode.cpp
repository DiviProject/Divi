...
bool CMnFunding::CheckVin() {
	CValidationState state;
	CMutableTransaction tx = CMutableTransaction();
	CTxOut vout = CTxOut(amount - 1000000, obfuScationPool.collateralPubKey);
...
bool CMasternode::VerifyFunding() {
	if (!unitTest) {
		CAmount totalFunding = 0;
		BOOST_FOREACH(CMnFunding& funds, funding) {
			if (GetInputAge(funds.vin) < MASTERNODE_MIN_CONFIRMATIONS) { activeState = ACTIVE_MASTERNODE_INPUT_TOO_NEW; return false; }
			// TODO: check signature and cache result
			if (!funds.CheckVin()) { activeState = MASTERNODE_VIN_SPENT; return false; }
			totalFunding += funds.amount;
		}
		if (totalFunding < tier.amount) { activeState = MASTERNODE_INSUFFICIENT_FUNDS; return false; }
	}
	return true;
}
...
void CMasternode::Check(bool forceCheck)
{
	...
	if (!IsPingedWithin(MASTERNODE_REMOVAL_SECONDS)) { activeState = MASTERNODE_REMOVE; return; }
	if (!IsPingedWithin(MASTERNODE_EXPIRATION_SECONDS)) { activeState = MASTERNODE_EXPIRED; return; }
	if (!VerifyFunding()) return;
...
int64_t CMasternode::GetLastPaid()
{
	...
	CScript mnpayee;
	mnpayee = GetScriptForDestination(pubKeyMasternode.GetID());
...
CMasternodeBroadcast::CMasternodeBroadcast(CService newAddr, CPubKey pubKeyMasternodeNew, int protocolVersionIn)
{
	addr = newAddr;
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet)
{
	...
	mnbRet = CMasternodeBroadcast(service, txin, pubKeyMasternodeNew, PROTOCOL_VERSION);
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
	...
	if (!obfuScationSigner.VerifyMessage(pubKeyMasternode, sig, strMessage, errorMessage)) {
	...
	if (pmn == NULL)
		...
		if (pmn->sigTime > sigTime) {
			LogPrint("masternode", "mnb - Bad sigTime %d for Masternode %s (existing broadcast is at %d)\n",
				sigTime, pubKeyMasternode.GetHex(), pmn->sigTime);
			return false;
		}
	...
	if (pmn->pubKeyMasternode == pubKeyMasternode && !pmn->IsBroadcastedWithin(MASTERNODE_MIN_MNB_SECONDS)) {
...
{
	// we are a masternode with the same pubKeyMasternode (i.e. already activated) and this mnb is ours (matches our Masternode key)
	// so nothing to do here for us
	if (fMasterNode && pubKeyMasternode == me.pubKeyMasternode) return true;
	...
	if (pmn != NULL) {
		...
		// if it's not enabled, remove old MN first and continue
			...
			mnodeman.Remove(pmn->pubKeyMasternode);
	}

	if (!VerifyFunding()) return false;

	LogPrint("masternode", "mnb - Accepted Masternode entry\n");

	// verify that sig time is legit in past
	// should be at least not earlier than block when 1000 DIV tx got MASTERNODE_MIN_CONFIRMATIONS
	uint256 hashBlock = 0;
	CTransaction tx2;
...
bool CMasternodeBroadcast::Sign(CKey& keyMasternode)
{
	std::string errorMessage;
	...
	if (!obfuScationSigner.SignMessage(strMessage, errorMessage, sig, keyMasternode)) {
...
CMasternodePing::CMasternodePing(CPubKey& pubKey)
{
	pubKey = CPubKey();
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
	...
	sigTime = GetAdjustedTime();
	std::string strMessage = pubKey.GetHex() + blockHash.ToString() + boost::lexical_cast<std::string>(sigTime);
...
bool CMasternodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled)
{
	...
	if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
		...
		if (!pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime)) {
			...
			LogPrint("masternode", "CMasternodePing::CheckAndUpdate - Masternode ping accepted, pubKey: %s\n", pubKey.GetHex());

			Relay();
			return true;
		}
		LogPrint("masternode", "CMasternodePing::CheckAndUpdate - Masternode ping arrived too early, pubKey: %s\n", pubKey.GetHex());
		//nDos = 1; //disable, this is happening frequently and causing banned peers
		return false;
	}
	LogPrint("masternode", "CMasternodePing::CheckAndUpdate - Couldn't find compatible Masternode entry, pubKey: %s\n", pubKey.GetHex());
