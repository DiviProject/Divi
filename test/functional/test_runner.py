...
`test/functional/test_framework/test_framework.py:DiviTestFramework.main`.
...
def main():
    ...
    enable_divid = config["components"].getboolean("ENABLE_BITCOIND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/divi/divi/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/divi/divi/pull/5677#issuecomment-136646964
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci):
    ...
    # Warn if divid is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "divid"]) is not None:
