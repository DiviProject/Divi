...
const char * const BITCOIN_CONF_FILENAME = "divi.conf";
const char * const BITCOIN_PID_FILENAME = "divid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "divid -foo=bar
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Divi";
#else
    // Unix
    return pathRet / ".divi";
#endif
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Divi Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Divi Core") == std::string::npos) {
