    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
        throw std::runtime_error(
            RPCHelpMan{"importaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The Divi address (or hex-encoded script)"},
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpprivkey",
                ...
                {
                    {"address", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The divi address for the private key"},
                }}
                .ToString() +
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"dumpwallet",
                ...
                {
                    {"filename", RPCArg::Type::STR, /* opt */ false, /* default_val */ "", "The filename with path (either absolute or relative to divid)"},
                }}
                .ToString() +
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/divi/divi/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Divi %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see divid log for details) and could "
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    ...
    auto pwalletMain = GetWalletForJSONRPCRequest(request);
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("mintablecoins", pwalletMain->MintableCoins(*pwalletMain->chain().lock())));
