...
std::string LicenseInfo()
{
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
		...
		FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Divi Core Developers"), COPYRIGHT_YEAR)) + "\n" + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
	...
	if (fMasterNode) {
		...
		std::string diviAddress = GetArg("-diviaddress", "");
		if (diviAddress.empty()) InitError(_("You must specify a divi address in the configuration. Please see documentation for help."));
		CBitcoinAddress addr(diviAddress);
		if (!addr.IsValid()) InitError(_("Invalid divi address in the configuration file."));
	}
