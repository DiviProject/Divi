...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " divi-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  divi-tx [options] <hex-tx> [commands]  Update hex-encoded divi transaction\n" +
            "or:     divi-tx [options] -create [commands]   Create hex-encoded divi transaction\n" +
            "\n";
...
static void MutateTxAddOutData(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CTxOut txout(value, CScript() << OP_META << data);
    tx.vout.push_back(txout);
}
