...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DIVX address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no DIVX: URI
...
{
    // Convert divx:// to divx:
    //
    //    Cannot handle this later, because divx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open divx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DIVX.lnk";
}
...
{
    return GetAutostartDir() / "divx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a divx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DIVX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the divx app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add divx app to startup item list
