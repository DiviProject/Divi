...
bool CMasternodeMan::Add(CMasternode& mn)
{
	...
	CMasternode* pmn = Find(mn.pubKeyMasternode);
	if (pmn == NULL) {
		LogPrint("masternode", "CMasternodeMan: Adding new Masternode %s - %i now\n", mn.pubKeyMasternode.GetHex(), size() + 1);
...
void CMasternodeMan::AskForMN(CNode* pnode, CPubKey& pubKey)
{
	std::map<CPubKey, int64_t>::iterator i = mWeAskedForMasternodeListEntry.find(pubKey);
...
//
//	BOOST_FOREACH(CMasternode& mn, vMasternodes) {
//		payee2 = GetScriptForDestination(mn.pubKeyMasternode.GetID());
//		if (payee2 == payee)
//			return &mn;
//	}
//	return NULL;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
	...
	if (strCommand == "mnb") { //Masternode Broadcast
		...
		if (!obfuScationSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubKeyMasternode)) {
		...
		if (nDoS > 0) {
			...
			CMasternode* pmn = Find(mnp.pubKey);
...
void CMasternodeMan::Remove(CPubKey pubKey)
{
	LOCK(cs);

	vector<CMasternode>::iterator it = vMasternodes.begin();
	while (it != vMasternodes.end()) {
		if ((*it).pubKeyMasternode == pubKey) {
			LogPrint("masternode", "CMasternodeMan: Removing Masternode %s - %i now\n", (*it).pubKeyMasternode.GetHex(), size() - 1);
