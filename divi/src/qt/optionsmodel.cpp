...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDivxAmount"))
        settings.setValue("nAnonymizeDivxAmount", 1000);

    nAnonymizeDivxAmount = settings.value("nAnonymizeDivxAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDivxAmount"))
        SoftSetArg("-anonymizedivxamount", settings.value("nAnonymizeDivxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in divx.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDivxAmount:
            return QVariant(nAnonymizeDivxAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDivxAmount:
            nAnonymizeDivxAmount = value.toInt();
            settings.setValue("nAnonymizeDivxAmount", nAnonymizeDivxAmount);
            emit anonymizeDivxAmountChanged(nAnonymizeDivxAmount);
            break;
        case CoinControlFeatures:
