...
static void FindNextBlocksToDownload(NodeId nodeid, unsigned int count, std::vector<const CBlockIndex*>& vBlocks, NodeId& nodeStaller, const Consensus::Params& consensusParams) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    while (pindexWalk->nHeight < nMaxHeight) {
        ...
        for (const CBlockIndex* pindex : vToFetch) {
            ...
            if (!State(nodeid)->fHaveWitness && IsWitnessEnabled(pindex->nHeight, consensusParams)) {
...
void PeerLogicValidation::NewPoWValidBlock(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock) {
    ...
    bool fWitnessEnabled = IsWitnessEnabled(pindex->nHeight, Params().GetConsensus());
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    return net_processing_divi::AlreadyHave(inv);
}
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        if (pblock) {
            ...
            if (inv.type == MSG_BLOCK && !pblock->IsWitnessBlock())
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !chainActive.Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        ...
                        (!IsWitnessEnabled(pindexWalk->nHeight, chainparams.GetConsensus()) || State(pfrom->GetId())->fHaveWitness)) {
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (nVersion < MIN_PEER_PROTO_VERSION || (chainparams.RequiresSamePeerVersion() && nVersion != PROTOCOL_VERSION)) {
        ...
        if(pfrom->fInbound) {
            connman->PushMessage(pfrom, CNetMsgMaker(MIN_PEER_PROTO_VERSION).Make(NetMsgType::SPORKCOUNT, sporkManager.GetActiveSporkCount()));
        }
...
//                    connman->PushMessage(pfrom, msgMaker.Make(NetMsgType::GETHEADERS, chainActive.GetLocator(pindexBestHeader), inv.hash));
    ...
    if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (IsWitnessEnabled(pindex->nHeight, chainparams.GetConsensus()) && !nodestate->fSupportsDesiredCmpctVersion) {
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
...
#if 0
                connman->PushMessage(pto, msgMaker.Make(NetMsgType::GETHEADERS, chainActive.GetLocator(pindexStart), uint256())); // old code, headers first sync
#endif
            }
        }
        ...
        std::vector<CInv> vInv;
        {
            ...
            {
                net_processing_divi::TransformInvForLegacyVersion(inv, pto, true);
                vInv.push_back(inv);
                if (vInv.size() == MAX_INV_SZ)
                {
                    connman->PushMessage(pto, msgMaker.Make(NetMsgType::INV, vInv));
