...
class SendMoneyOptions (BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.sender.sendtoaddress(self.receiver.getnewaddress(),send_less_than_all,"send_to_self")
        sync_mempools(self.nodes)
        self.minter.setgenerate(1)
        self.sync_all()
        remaining_balance_after_send_to_self = self.sender.getbalance()
        print("SEND_TO_SELF mode:\n\tStarted with {}, sent {}, remaining with {}".format(sender_remaining_balance, send_less_than_all, remaining_balance_after_send_to_self))
